...
def create_alert_email_body(alert_data):
    """Creates the HTML email body for a transaction alert."""
    return f"""<html><body style="font-family: Arial, sans-serif;">
    <h2 style="color: #d9534f;">⚠️ Transaction Limit Exceeded Alert</h2>
    ...
    This is an automated message from FraudWatch Transaction Monitoring System.
    </p>
    </body></html>"""
...
def send_limit_exceeded_alert_emails(df, batch_id):
    """ForEachBatch sink that sends email alerts for limit-exceeded transactions."""
    ...
    for row in rows:
        try:
            ...
            subject = f"⚠️ Limit-Exceeded Transaction Alert - {alert_data['alert_id']}"
...
def limit_exceeded_alert_stream():
    """Streaming flow that reads high-value transaction alerts."""
    return spark.readStream.table("fraudwatch.gold.transaction_alerts")
